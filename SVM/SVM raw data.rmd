...
# Load data
```{r}
diabetic.data <- read.csv('../data/train_raw.csv', header=TRUE)
...
diabetic.no <- diabetic.no[1:15000,]

diabetic.less30 <- subset(diabetic.data, readmitted == "<30")
diabetic.less30 <-diabetic.less30[1:15000,]

diabetic.more30 <- subset(diabetic.data, readmitted == ">30")
diabetic.more30 <- diabetic.more30[1:15000,]
...
all.metrics
saveRDS(all.metrics, file = "output/metrics_raw_data.rds")
```
...
classifier = svm(formula = readmitted ~ .,
                   ...
                   kernel = 'radial')
print(Sys.time()-start_time)
saveRDS(classifier, file = "output/model_raw_data.rds")
...
# Predict test data
start_time = Sys.time()
pred_svm_test_train_raw <- predict(classifier, newdata = df_test)
pred_svm_test_train_raw <- factor(pred_svm_test_train_raw, order = TRUE, levels = c("NO", ">30", "<30"))
print(Sys.time()-start_time)

#Evaluate prediction 
cm_test_raw <- confusionMatrix(pred_svm_test_train_raw, df_test$readmitted)
cm_test_raw
saveRDS(cm_test_raw, file = "output/cm_test_raw_data.rds")
